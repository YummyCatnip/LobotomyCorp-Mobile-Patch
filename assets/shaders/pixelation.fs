...
extern MY_HIGHP_OR_MEDIUMP vec2 pixelation;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    // 0.16 / 0.1 / 0.05 is good
    // 0.4 / 0.5 / 0.25 is good
    float pixels = 0.16;
    float dx = 0.2 * (0.01 / pixels);
    float dy = 0.1 * (0.01 / pixels);
    vec2 coord = vec2(dx * floor((texture_coords.x / dx) + 0.5), dy * floor((texture_coords.y / dy) + 0.5));
    vec4 tex = Texel(texture, coord);
    // if it hits a transparent pixel, don't
    if (tex.a == 0.0){
        coord = vec2(dx * ceil(texture_coords.x / dx), dy * ceil(texture_coords.y / dy));
        tex = Texel(texture, coord);
    }
    if (tex.a == 0.0){
        coord = vec2(dx * floor(texture_coords.x / dx), dy * floor(texture_coords.y / dy));
        tex = Texel(texture, coord);
    }
